...
namespace CodingBackProp
{
	...
	public class NeuralNetwork
	{
		  ...
		  double learnRate, double momentum)
		{
			...
			double[] oGradients = new double[numOutput];                  // local gradient output signals - gradients w/o associated input terms
			double[] hGradients = new double[numHidden];                  // local gradient hidden node signals
			...
			while (epoch < maxEpochs)
			{
				...
				for (int ii = 0; ii < trainData.Length; ++ii)
				{
					...
					for (int k = 0; k < numOutput; ++k)
					{
						...
						oGradients[k] = errorSignal * derivative;
					}

					// 2. compute hidden-to-output weight gradients using output signals
					for (int j = 0; j < numHidden; ++j)
						for (int k = 0; k < numOutput; ++k)
							hoGrads[j][k] = oGradients[k] * hOutputs[j];
					...
					for (int k = 0; k < numOutput; ++k)
						obGrads[k] = oGradients[k] * 1.0; // dummy assoc. input value
					...
					for (int j = 0; j < numHidden; ++j)
					{
						...
						for (int k = 0; k < numOutput; ++k)
						{
							sum += oGradients[k] * hoWeights[j][k]; // represents error signal
						}
						hGradients[j] = derivative * sum;
					}

					// 4. compute input-hidden weight gradients
					for (int i = 0; i < numInput; ++i)
						for (int j = 0; j < numHidden; ++j)
							ihGrads[i][j] = hGradients[j] * inputs[i];

					// 4b. compute hidden node bias gradients
					for (int j = 0; j < numHidden; ++j)
						hbGrads[j] = hGradients[j] * 1.0; // dummy 1.0 input
